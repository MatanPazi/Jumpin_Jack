...
float RPS = 0.1;              //Rotations per second
float Amp = 1.0;              //Amplitude of sine waves
float Deg = 30.0;            //Degree of rotation
//
...
//  WaitTime = 15625 / ((uint32_t)(Base_Freq / Desired_Freq));    //Wait 1 sec. OVF every 64 [us] -> 15625*64 = 1e6[us] = 1[sec] & counter will increment only if OVF_Counter > (Base_Freq / Desired_Freq).
  WaitTime = 3000;//15625;                                             //Wait 1 sec. OVF every 64 [us] -> 15625*64 = 1e6[us] = 1[sec]
  ...
  else
  {
    ...
    if (OVF_Counter >= ((unsigned long)(Base_Freq / Desired_Freq)))
    {
      ...
      else
      {
        ...
        OCR1B = 255;
        OCR2B = 255;
        Sine_Index_Counter = Deg_In_Index + 1;
      }
      OVF_Counter = 0;
    }
  }
